...
class SecondSingleReplace : LogicOperator {
	public static let INSTANCE = SecondSingleReplace()
	...
	public func operate(_ stack:inout [AnyObject], _ sc:Scope, _ stream:LogicStream) {
		...
		if plasiArg == nil {
			...
			if plasi!.selectAndReplace(base:base, move:LogicMove(LogicMove.ONE_IN_SECOND, side, position), sel:sc.getCurrentSelection(), check:proof.hasChecks()) == LogicMove.LEGAL {
				stack.append(plasi!)
				return
			}
			else {
