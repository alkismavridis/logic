class SecondPhraseReplace : LogicOperator {
	public static let INSTANCE = SecondPhraseReplace()
	...
	public func operate(_ stack:inout [AnyObject], _ sc:Scope, _ stream:LogicStream) {
		...
		if plasiArg == nil {
			...
			if plasi!.selectAndReplace(base:base, move:LogicMove(LogicMove.SECOND, side), sel:sc.getCurrentSelection(), check:proof.hasChecks()) == LogicMove.LEGAL {
				stack.append(plasi!)
				return
			}
			else {
